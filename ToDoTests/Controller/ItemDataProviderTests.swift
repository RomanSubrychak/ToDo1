//
//  temListDataProviderTests.swift
//  ToDoTests
//
...
class ItemDataProviderTests: XCTestCase {
	...
	func test_NumberOfRows_InSecondSection_IsDonCount() {
		sut.itemManager?.add(ToDoItem(title: "Foo"))
		sut.itemManager?.add(ToDoItem(title: "Bar"))
	...
	func test_CellForRowAt_InSectionTwo_CallsConfigCellWithDoneItem() {
		let mockTableView = MockTableView.mockTableView(with: sut)
	...
	func test_UncheckignAnItem_UnchecksItInItemManager() {
	...
	func test_SelectingCell_SendsNotification() {
		let item = ToDoItem(title: "Foo")
		sut.itemManager?.add(item)
...
extension ItemDataProviderTests {
