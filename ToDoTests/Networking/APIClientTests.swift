...
class APIClientTests: XCTestCase {
	...
	func test_Login_UsesExpectedURL() {
		let sut = APIClient()
		let mockURLSession = MockURLSession(data: nil, urlResoponse: nil, error: nil)
	...
	func test_Login_WhenSuccessfull_CreatesToken() {
		let sut = APIClient()
		let jsonData = "{\"token\": \"1234567890\"}".data(using: .utf8)
		let mockURLSession = MockURLSession(data: jsonData, urlResoponse: nil, error: nil)
	...
	func test_WhenJSONIsInvalid_ReturnsError() {
		let sut = APIClient()
		let mockURLSession = MockURLSession(data: Data(), urlResoponse: nil, error: nil)
	...
	func test_Login_WhenResponseHasError_ReturnsError() {
		...
		let jsonData = "{\"token\": \"1234567890\"}".data(using: .utf8)
		let mockSession = MockURLSession(data: jsonData, urlResoponse: nil, error: error)
...
extension APIClientTests {
	...
	class MockURLSession: SessionProtocol {
		...
		init(data: Data?, urlResoponse: URLResponse?, error: Error?) {
			self.dataTask = MockTask(data: data, urlResponse: urlResoponse, error: error)
		}
